...
import type { BaseRedemptionRequest } from "@/types/centralModels";
import { 
  getRedemptionRequests, 
  redemptionRequestToCsv, 
  redemptionRequestsToCsv 
} from "@/lib/redemptions";
import { mapDbRedemptionToRedemptionRequest } from "@/utils/typeMappers";
import { useToast } from "@/components/ui/use-toast";

// We need a common type definition to avoid mismatches
type RedemptionRequest = Omit<BaseRedemptionRequest, "createdAt" | "updatedAt"> & {
  id: string;
  requestDate: string;
};
...
const RedemptionRequestList: React.FC<RedemptionRequestListProps> = ({
  ...
  const loadRedemptionRequests = async () => {
    ...
    try {
      ...
      // Cast to the correct type after mapping
      setRequests(data.map(mapDbRedemptionToRedemptionRequest) as RedemptionRequest[]);
    } catch (error) {
  ...
  const handleExportSingle = (requestId: string) => {
    const requestToExport = requests.find(req => req.id === requestId);
    if (requestToExport) {
      // Using the properly typed request
      const csvContent = redemptionRequestToCsv(requestToExport);
      downloadCsv(csvContent, `redemption-${requestId}.csv`);
    }
  };

  const handleExportAll = () => {
    // Using the properly typed array
    const csvContent = redemptionRequestsToCsv(requests);
    downloadCsv(csvContent, "all-redemptions.csv");
  };
                    ...
                    <div className="flex justify-end">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem
                            onClick={() => handleExportSingle(request.id)}
                          >
                            Export Data
                          </DropdownMenuItem>
                        </DropdownMenuContent>
