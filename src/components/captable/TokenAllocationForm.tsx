...
import { getTemplatesWithStatus } from "@/lib/services/tokenTemplateService";
...
const TokenAllocationForm = ({
  ...
  const fetchAvailableTokenOptions = async () => {
    try {
      ...
      for (const status of eligibleStatuses) {
        try {
          const templates = await getTemplatesWithStatus(projectId, status);
          if (templates && templates.length > 0) {
            allTemplates = [...allTemplates, ...templates];
          }
        } catch (err) {
          console.error(`Error fetching templates with status ${status}:`, err);
  ...
  const addTokenAllocation = () => {
    ...
    currentAllocations.push({ token_type: "", token_amount: 0 });
