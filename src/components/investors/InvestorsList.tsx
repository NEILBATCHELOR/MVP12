...
import BatchScreeningDialog from "@/components/dashboard/BatchScreeningDialog";
    ...
    <>
      ...
      <Dialog open={isBulkUploadOpen} onOpenChange={setIsBulkUploadOpen}>
        <DialogContent className="sm:max-w-[900px] p-0 bg-white">
          <DialogHeader>
            <DialogTitle>Bulk Investor Upload</DialogTitle>
            <DialogDescription>Upload multiple investors at once with a CSV file</DialogDescription>
          </DialogHeader>
          <BulkInvestorUpload
      ...
      <BatchScreeningDialog
        ...
        investors={investors
          .filter((investor) => selectedInvestors.includes(investor.id))
          .map((investor) => ({
            id: investor.id,
            name: investor.name,
            email: investor.email,
            ...
            lastUpdated: investor.updated_at || new Date().toISOString(),
          }))}
        onScreen={async (investorIds, status) => {
          await handleScreeningComplete();
        }}
      />
