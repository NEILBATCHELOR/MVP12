...
// Interface for the token data in database response
...
const TokenAdministration: React.FC<TokenAdministrationProps> = ({
  ...
  const fetchTokens = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .eq("status", TokenStatus.READY_TO_MINT)
      ...
      const tokensWithSupply = (data || []).map((token) => ({
        ...token,
        total_supply: (token as TokenResponse).total_supply || 0,
      }));
      setTokens(tokensWithSupply as Token[]);

      // Select the first token by default if available
      if (data && data.length > 0 && !selectedToken) {
        setSelectedToken({
          ...data[0],
          total_supply: (data[0] as TokenResponse).total_supply || 0,
        } as Token);
      }
    } catch (err) {
  ...
  const handleProjectChange = (newProjectId: string) => {
    navigate(`/projects/${newProjectId}/token-admin`);
  };
  ...
  const fetchPrimaryProject = async () => {
    try {
      ...
      if (error) {
        ...
        if (error.code === 'PGRST116') {
          ...
          if (anyProject) {
            ...
            navigate(`/token-administration/${anyProject.id}`);
          }
        }
        return;
      }
      ...
      if (data) {
        ...
        navigate(`/token-administration/${data.id}`);
      }
    } catch (err) {
