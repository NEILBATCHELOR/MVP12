...
import type { PolicyTemplateInsert, PolicyTemplateUpdate, PolicyTemplateTable, TemplateVersionTable, TemplateVersionInsert } from '@/types/database';
 ...
 */
export async function getAllPolicyTemplates(): Promise<PolicyTemplateTable[]> {
 ...
 */
export async function getPolicyTemplateById(templateId: string): Promise<PolicyTemplateTable | null> {
  const { data, error } = await supabase
    .from('policy_templates')
    .select('*')
 ...
 */
export function templateToPolicy(template: PolicyTemplateTable): Policy {
