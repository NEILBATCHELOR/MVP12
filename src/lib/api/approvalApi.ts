...
import { BaseRedemptionRequest, Approver } from "@/types/centralModels";
import type { 
...
export const getAllRedemptionRequests = async (): Promise<RedemptionRequest[]> => {
  try {
    ...
    return (data as (RedemptionRequestTable & { redemption_approvers: RedemptionApproverTable[] })[]).map(request => ({
      id: request.id,
...
export const getRedemptionRequestById = async (id: string): Promise<RedemptionRequest | null> => {
  try {
    ...
    const request = data as RedemptionRequestTable & { redemption_approvers: RedemptionApproverTable[] };
