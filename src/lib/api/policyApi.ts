...
import { ApiResponse, handleApiError } from '@/lib/api/index';
import { 
  getPolicy, 
  getAllPolicies, 
  savePolicy as savePolicyService,
  type Policy
} from '@/services/enhancedPolicyService';
import { 
...
import type { PolicyTemplateTable } from '@/types/database';
 ...
 */
export async function getPolicyTemplates(): Promise<ApiResponse<PolicyTemplateTable[]>> {
  try {
...
export async function getPolicyTemplateByIdApi(
  templateId: string
): Promise<ApiResponse<PolicyTemplateTable | null>> {
  try {
...
export async function createPolicyTemplate(
  ...
  policyData: Policy,
  userId: string
): Promise<ApiResponse<PolicyTemplateTable>> {
  try {
    // Validate permissions (would typically be done by middleware)
    const { data: userRoles, error: rolesError } = await supabase
      .from('user_roles')
