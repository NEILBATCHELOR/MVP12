...
export abstract class FeeEstimator {
  ...
  protected historicalFees: Map<number, ethers.BigNumber> = new Map();
...
export class EVMFeeEstimator extends FeeEstimator {
  ...
  async estimateFee(options?: FeeEstimationOptions): Promise<FeeSuggestion> {
    ...
    try {
      ...
      if (feeData.maxFeePerGas && feeData.maxPriorityFeePerGas) {
        ...
        suggestion.baseFee = ethers.utils.formatUnits(baseFee, "gwei");
        suggestion.priorityFee = ethers.utils.formatUnits(feeData.maxPriorityFeePerGas, "gwei");
        
        // Apply priority multipliers
        const priorityMultiplier = this.getPriorityMultiplier(priority, congestion);
        const adjustedPriorityFee = feeData.maxPriorityFeePerGas.mul(priorityMultiplier).div(10);
        
        suggestion.maxPriorityFeePerGas = ethers.utils.formatUnits(adjustedPriorityFee, "gwei");
        ...
        const adjustedMaxFee = baseFee.mul(maxFeeMultiplier).div(10).add(adjustedPriorityFee);
        
        suggestion.maxFeePerGas = ethers.utils.formatUnits(adjustedMaxFee, "gwei");
        suggestion.maxFeePerGasWei = adjustedMaxFee.toString();
      } else if (feeData.gasPrice) {
        // Legacy fee structure
        suggestion.gasPrice = ethers.utils.formatUnits(feeData.gasPrice, "gwei");
        suggestion.gasPriceWei = feeData.gasPrice.toString();
        
        // Apply priority multiplier
        const multiplier = this.getPriorityMultiplier(priority, congestion);
        const adjustedGasPrice = feeData.gasPrice.mul(multiplier).div(10);
        
        suggestion.gasPrice = ethers.utils.formatUnits(adjustedGasPrice, "gwei");
  ...
  async getNetworkCongestion(): Promise<NetworkCongestion> {
    try {
      ...
      if (latestBlock.baseFeePerGas) {
        // Thresholds in gwei
        const lowThreshold = ethers.utils.parseUnits("20", "gwei");
        const mediumThreshold = ethers.utils.parseUnits("100", "gwei");
        const highThreshold = ethers.utils.parseUnits("200", "gwei");
        
        if (latestBlock.baseFeePerGas.lt(lowThreshold)) {
          return NetworkCongestion.LOW;
        } else if (latestBlock.baseFeePerGas.lt(mediumThreshold)) {
          return NetworkCongestion.MEDIUM;
        } else if (latestBlock.baseFeePerGas.lt(highThreshold)) {
        ...
        const gasUsageRatio = latestBlock.gasUsed.mul(100).div(latestBlock.gasLimit).toNumber();
   ...
   */
  private updateHistoricalFees(feeData: ethers.providers.FeeData): void {
