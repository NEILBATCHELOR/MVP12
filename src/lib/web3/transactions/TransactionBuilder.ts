...
export abstract class BaseTransactionBuilder implements TransactionBuilder {
  ...
  protected increaseFeeForCancel(transaction: Transaction): string {
    ...
    const currentGasPrice = ethers.BigNumber.from(transaction.gasPrice);
  ...
  protected increaseFeeForSpeedup(transaction: Transaction, priorityLevel: TransactionPriority): string {
    ...
    let increasedGasPrice: ethers.BigNumber;
    
    switch (priorityLevel) {
      case TransactionPriority.LOW:
        increasedGasPrice = currentGasPrice.mul(120).div(100); // 20% increase
        break;
      case TransactionPriority.MEDIUM:
        increasedGasPrice = currentGasPrice.mul(150).div(100); // 50% increase
        break;
      case TransactionPriority.HIGH:
        increasedGasPrice = currentGasPrice.mul(200).div(100); // 100% increase
        break;
      case TransactionPriority.URGENT:
        increasedGasPrice = currentGasPrice.mul(300).div(100); // 200% increase
        break;
      default:
        increasedGasPrice = currentGasPrice.mul(150).div(100); // 50% default
    }
