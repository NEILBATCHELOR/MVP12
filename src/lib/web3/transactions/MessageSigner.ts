...
export class EthereumMessageSigner implements MessageSigner {
  ...
  async signMessage(
    ...
    switch (type) {
      case MessageSigningType.ETH_SIGN:
        ...
        signature = await wallet.signMessage(ethers.utils.arrayify(messageHash));
        break;
  ...
  async signTypedData(
    ...
    // Use ethers._signTypedData for v4
    if (version === "v4") {
      const signature = await wallet._signTypedData(domain, typesForSigning, message);
      messageHash = ethers.utils._TypedDataEncoder.hash(domain, typesForSigning, message);
