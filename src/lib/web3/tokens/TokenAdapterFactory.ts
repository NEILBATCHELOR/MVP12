...
export class TokenAdapterFactory implements ITokenAdapterFactory {
  private adapters: Map<string, new (provider: ethers.providers.Provider) => TokenAdapter> = new Map();
  ...
  registerAdapter(
    standard: string, 
    adapter: new (provider: ethers.providers.Provider) => TokenAdapter
  ): void {
   ...
   */
  async createAdapter(tokenAddress: string, provider: ethers.providers.Provider): Promise<TokenAdapter> {
    ...
    if (!AdapterClass) {
      if (standard === "ERC20") {
        // Default to ERC20 adapter (not implemented here, would need to be created)
        throw new Error("ERC20 adapter not implemented yet");
   ...
   */
  createAdapterForStandard(standard: string, provider: ethers.providers.Provider): TokenAdapter {
    const AdapterClass = this.adapters.get(standard);
