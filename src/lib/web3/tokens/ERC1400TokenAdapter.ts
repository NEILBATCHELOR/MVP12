...
import { ERC1400_ABI } from "@/lib/web3/TokenInterfaces";
...
export class ERC1400TokenAdapter extends BaseTokenAdapter {
  ...
  async getTokenMetadata(tokenAddress: string): Promise<Token> {
    const contract = new ethers.Contract(tokenAddress, ERC1400_ABI, this.provider);
  ...
  async getPartitions(tokenAddress: string): Promise<string[]> {
    ...
    try {
      return await contract.totalPartitions();
    } catch (error) {
  ...
  async getPartitionsOf(tokenAddress: string, ownerAddress: string): Promise<string[]> {
    ...
    try {
      return await contract.partitionsOf(ownerAddress);
    } catch (error) {
  ...
  async getDocuments(tokenAddress: string): Promise<SecurityTokenDocument[]> {
    ...
    try {
      ...
      const commonDocNames = [
        ethers.utils.formatBytes32String("whitepaper"),
        ethers.utils.formatBytes32String("prospectus"),
        ethers.utils.formatBytes32String("legal"),
        ethers.utils.formatBytes32String("disclaimer"),
        ethers.utils.formatBytes32String("issuer")
      ];
      ...
      for (const docName of commonDocNames) {
        try {
          ...
          if (uri) {
            documents.push({
              name: ethers.utils.parseBytes32String(docName),
              uri,
              documentHash
            });
          }
  ...
  async transferByPartition(
    ...
    try {
      ...
      // Transfer the tokens
      const tx = await contractWithSigner.transferByPartition(
        partition,
        toAddress,
        amount,
        data
  ...
  async estimateTransferByPartitionGas(
    ...
    try {
      ...
      const gasEstimate = await contract.estimateGas.transferByPartition(
        partition,
        toAddress,
        amount,
        data
