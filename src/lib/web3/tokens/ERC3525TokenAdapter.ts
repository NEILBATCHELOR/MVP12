...
import { ERC3525_ABI } from "@/lib/web3/TokenInterfaces";
...
export class ERC3525TokenAdapter extends BaseTokenAdapter {
  ...
  async getTokenMetadata(tokenAddress: string): Promise<Token> {
    const contract = new ethers.Contract(tokenAddress, ERC3525_ABI, this.provider);
  ...
  private async getTokenValue(contract: ethers.Contract, tokenId: string): Promise<ethers.BigNumber> {
    ...
    try {
      ...
      return ethers.BigNumber.from(1000); // Mock value for testing
    } catch (error) {
  ...
  async approve(
    ...
    try {
      ...
      // Approve value
      const tx = await contractWithSigner.approve(tokenId, spenderAddress, amount);
  ...
  async transferValue(
    ...
    try {
      ...
      // Transfer value
      const tx = await contractWithSigner.transferValue(fromTokenId, toTokenId, amount);
      const receipt = await tx.wait();
  ...
  async estimateTransferValueGas(
    ...
    try {
      ...
      // Estimate gas
      const gasEstimate = await contract.estimateGas.transferValue(
        fromTokenId,
        toTokenId,
        amount
      );
  ...
  async supportsInterface(tokenAddress: string, interfaceId: string): Promise<boolean> {
    ...
    try {
      const contract = new ethers.Contract(tokenAddress, erc165Interface, this.provider);
