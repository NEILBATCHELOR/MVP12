...
import { ERC1155_ABI } from "@/lib/web3/TokenInterfaces";
...
export class ERC1155TokenAdapter extends BaseTokenAdapter {
  ...
  async getTokenMetadata(tokenAddress: string): Promise<Token> {
    ...
    const nameSymbolInterface = new ethers.utils.Interface([
      "function name() view returns (string)",
      "function symbol() view returns (string)"
    ]);
    
    try {
      const contract = new ethers.Contract(tokenAddress, nameSymbolInterface, this.provider);
  ...
  async estimateTransferGas(
    ...
    try {
      const gasEstimate = await contract.estimateGas.safeTransferFrom(
        fromAddress,
        toAddress,
        tokenId,
        amount,
  ...
  async getTokenMetadataById(tokenAddress: string, tokenId: string): Promise<NFTMetadata> {
    ...
    try {
      ...
      if (uri.includes("{id}")) {
        ...
        const hexId = ethers.BigNumber.from(tokenId).toHexString();
        const paddedHexId = hexId.substring(2).padStart(64, "0");
        uri = uri.replace("{id}", paddedHexId);
      }
