...
import { ERC4626_ABI } from "@/lib/web3/TokenInterfaces";
...
export class ERC4626TokenAdapter extends BaseTokenAdapter {
  ...
  async getTokenMetadata(tokenAddress: string): Promise<Token> {
    const contract = new ethers.Contract(tokenAddress, ERC4626_ABI, this.provider);
  ...
  async getVaultInfo(tokenAddress: string): Promise<VaultToken> {
    ...
    try {
      ...
      const assetContract = new ethers.Contract(assetAddress, erc20Interface, this.provider);
      ...
      if (totalSupply.gt(0)) {
        ...
        sharePrice = totalAssets.mul(ethers.BigNumber.from(10).pow(basicTokenInfo.decimals))
          .div(totalSupply)
          .toString();
      }
      ...
      try {
        ...
        const apyInterface = new ethers.utils.Interface([
          "function getAPY() view returns (uint256)"
        ]);
        
        const apyContract = new ethers.Contract(tokenAddress, apyInterface, this.provider);
        const rawApy = await apyContract.getAPY();
        apy = ethers.utils.formatUnits(rawApy, 4) + "%"; // Assuming 4 decimal places for percentage
      } catch (e) {
  ...
  async deposit(
    ...
    try {
      ...
      const assetContract = new ethers.Contract(assetAddress, erc20Interface, wallet);
      const approveTx = await assetContract.approve(tokenAddress, assetAmount);
      await approveTx.wait();
      
      // Deposit assets into the vault
      const tx = await contractWithSigner.deposit(assetAmount, ownerAddress);
  ...
  async mint(
    ...
    try {
      ...
      // Mint shares
      const tx = await contractWithSigner.mint(shareAmount, ownerAddress);
      const receipt = await tx.wait();
  ...
  async estimateTransferGas(
    ...
    try {
      const gasEstimate = await contract.estimateGas.transfer(toAddress, amount, { from: fromAddress });
  ...
  async estimateDepositGas(
    ...
    try {
      ...
      if (ethers.BigNumber.from(assetAmount).gt(maxDeposit)) {
        throw new Error(`Deposit amount exceeds maximum allowed (${maxDeposit.toString()})`);
      }
      
      const gasEstimate = await contract.estimateGas.deposit(assetAmount, ownerAddress, { from: ownerAddress });
  ...
  async estimateWithdrawGas(
    ...
    try {
      ...
      if (ethers.BigNumber.from(assetAmount).gt(maxWithdraw)) {
