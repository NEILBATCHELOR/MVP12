...
export interface TokenAdapterFactory {
   ...
   */
  createAdapter(tokenAddress: string, provider: ethers.providers.Provider): Promise<TokenAdapter>;
  
  /**
   * Register a new token adapter
   */
  registerAdapter(standard: string, adapter: new (provider: ethers.providers.Provider) => TokenAdapter): void;
}
...
export async function detectTokenStandard(
  ...
  try {
    ...
    const contract = new ethers.Contract(tokenAddress, erc165Interface, provider);
  ...
  try {
    ...
    try {
      const erc4626Interface = new ethers.utils.Interface([
        "function asset() view returns (address)",
        "function totalAssets() view returns (uint256)"
      ]);
      
      const vaultContract = new ethers.Contract(tokenAddress, erc4626Interface, provider);
    ...
    try {
      const erc1400Interface = new ethers.utils.Interface([
        "function getDocument(bytes32 name) view returns (string, bytes32)",
        "function isControllable() view returns (bool)"
      ]);
      
      const securityTokenContract = new ethers.Contract(tokenAddress, erc1400Interface, provider);
