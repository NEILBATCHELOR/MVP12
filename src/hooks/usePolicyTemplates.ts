...
import type { PolicyTemplateWithData, PolicyTemplateData } from '@/types/policyTemplates';
...
export function usePolicyTemplates() {
  ...
  // Update a template
  const updateTemplate = useCallback(async (id: string, updates: Partial<PolicyTemplateTable>) => {
    try {
      setLoading(true);
      setError(null);
  ...
  const convertTemplateToPolicy = useCallback((template: PolicyTemplateWithData): Policy => {
    return templateToPolicy(template as unknown as PolicyTemplateTable);
  }, []);
